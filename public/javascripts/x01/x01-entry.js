...
function EntryX01(parentLeg, index) {
    ...
    for (var i = 0; i < players.length; i++) {
        var p = players[i];
        var score = parent.getPlayerScore(p);
        playerLeft[p.uuid] = score;
        playerPreviousLeft[p.uuid] = score;
    }
    ...
    this.next = function (callback) {
        ...
        this.activatePlayer(previousPlayer, callback);
    };
    this.close = function () {
    ...
    /** Show Finish */
    this.showFinish = function (player, score) {
        appendFinish($("#" + parent.getFinishHelperId(player)), score);
    };
    /** Player activation */
    this.activatePlayer = function (previousPlayer, callback) {
        // Fix size
        var w = $(window).width();
        if (w < 768) { // FIXME do in Resize
            ...
            var $submit;
            for (var i = 0; i < players.length; i++) {
                $submit = $("#" + parent.getSubmitPlayer(players[i]));
        ...
        if (previousPlayer && previousPlayer.uuid) {
            var $accPrevious = $("#" + parent.getStatsPlayerId(previousPlayer)).parent();
            if ($accPrevious.hasClass("collapse")) {
                $accPrevious.collapse("hide")
                    .one("hidden", function () {
                        var $acc = $("#" + parent.getStatsPlayerId(lastPlayer)).parent();
                        $acc.collapse("show");
                    });
            }
            entry.askNewInput(callback);
        } else {
            // New Player
            var $acc = $("#" + parent.getStatsPlayerId(lastPlayer)).parent();
            if ($acc.hasClass("collapse")) {
    ...
    this.askComputerPlayer = function ($input, callback) {
        ...
        var $dialog = $("#computerThrowDialog");
        $dialog.find(".done").empty();
        $dialog.find(".wished").empty();
        $dialog.find("h3").html(msgTitle);
    ...
    this.showDart = function (entry, json, idx, callback) {
        ...
        if ($.isPlainObject(dart)) {
            ...
            $dialog.find(".wished").append($badgeWished);

            var $badgeDone = $("<div/>").addClass("span4").addClass(dart.color).append(dart.done);
            $dialog.find(".done").append($badgeDone);
    ...
    this.handleNewInput = function (status, value, callback) {
        ...
        var $left = $("#" + this.getLeftId(lastPlayer));
        $left.html(pLeft);
        if (isSpecial(pLeft)) {
            $left.addClass("special");
        }

        var $score = $("#" + this.getScoreId(lastPlayer));
        $score.addClass(status).html(this.getScore(lastPlayer)).addClass(function () {
    ...
    this.destroyPlayer = function (player) {
        ...
        var $score = $("#" + this.getScoreId(player));

        $left.html(this.getLeft(player));
        $score.html(this.getScore(player));
        if (!player.com) {
