...
var openModalDialog = function (title, message, buttons) {
    ...
    $dialog.find(".modal-body p").empty().append(message);

    var $buttonsBar = $dialog.find(".modal-footer");
    $buttonsBar.empty();
    ...
    $dialog.one("shown",function () {
        $dialog.find(".modal-footer button:last-child").focus();
    }).modal("show");
};
...
var updatePlayerList = function (prefix) {
    ...
    var $dialog = $("#diaPlayerSelect");
    $dialog.find(".btn-primary").attr("disabled", "disabled");
    $dialog.val("");
    $dialog.find(".playerList").empty();
            ...
            $("<li/>").append(
                $("<a/>").attr("href", "#").append(player.getDisplayName()).click(function (event) {
                    if ($(this).parent().hasClass("active")) {
                        ...
                        $dialog.find(".btn-primary").attr("disabled", "disabled");
                        $dialog.find(".playerList li").removeClass("active");
                    } else {
...
var createPlayer = function (callback) {
    ...
    $dialog.find("form").one("submit", function (event) {
        doCreatePlayer(event, callback);
    });
    $dialog.modal("show");
};
...
var doCreatePlayer = function (event, callback) {
    ...
    if (isComputer) {
        ...
        player.comTarget = $dialog.find(".btnTarget .active").html();
