...
sealed abstract class DartChoice {
  ...
  def allDarts: List[(Dart, Set[Modifier])]
...
case class OrChoice(dartChoices: DartChoice*) extends DartChoice {
  lazy val allDarts = dartChoices.flatMap(_.allDarts).toList
...
case class AlwaysDart(dart: Dart) extends DartChoice {
  val allDarts = {
    val modifier: Modifier = DefaultCase
    List((dart, Set(modifier)))
  }
...
case class OrDart(darts: Dart*) extends DartChoice {
  ...
  lazy val allDarts = {
    val modifier: Modifier = DefaultCase
    darts.map(d => (d, Set(modifier))).toList
  }
  ...
  def getDartsWeight(modifiers: Set[Modifier]): Seq[(Dart, Int)] = darts.toList map (getDartWeight(_, modifiers))
...
case class PreferedDartBut(preferedChoice: DartChoice, otherChoice: DartChoice) extends DartChoice {
  ...
  lazy val allDarts = preferedChoice.allDarts ::: {
    for {
      (dart, modifiers) <- otherChoice.allDarts
    } yield (dart, modifiers + Optional)
  }
...
abstract class OnModifierDartsChoice(modifier: Modifier, onModifierChoice: DartChoice, elseChoice: DartChoice) extends DartChoice {
  lazy val allDarts = {
    val withModifier = for {
      (dart, modifiers) <- onModifierChoice.allDarts
    } yield (dart, modifiers + modifier)

    elseChoice.allDarts ::: withModifier
  }
